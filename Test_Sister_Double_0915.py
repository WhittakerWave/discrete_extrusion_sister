...
with open("data/extrusion_dict_RN_RB_RP_RW_HBD_S6.json", 'r') as dict_file:
...
monomers_per_replica = paramdict_dW['monomers_per_replica'] 
sites_per_monomer = paramdict_dW['sites_per_monomer']
...
LEF_off_rate = paramdict_dW['LEF_off_rate']
CTCF_facestall = paramdict_dW['CTCF_facestall']
...
def apply_param_dict_to_translocator(translocator, param_dict):
    """Apply parameters from dictionary to translocator"""
    if hasattr(translocator, 'params'):
        translocator.params.update(param_dict)
    
    # Apply parameters to engines if they have methods to update them
    if hasattr(translocator.extrusion_engine, 'update_params'):
        translocator.extrusion_engine.update_params(param_dict)
    elif hasattr(translocator.extrusion_engine, 'set_params'):
        translocator.extrusion_engine.set_params(param_dict)
    
    if hasattr(translocator.barrier_engine, 'update_params'):
        translocator.barrier_engine.update_params(param_dict)
    elif hasattr(translocator.barrier_engine, 'set_params'):
        translocator.barrier_engine.set_params(param_dict)
...
def run_synchronized_trajectories_simple(translocator1, translocator2, period = None, steps = None, 
                                        ...
                                        sample_interval=1, seed=42, **kwargs):
    # Pre-generate shared decay decisions
    num_sisters = translocator1.extrusion_engine.num_sisters
    sister_tau = translocator1.extrusion_engine.sister_tau
    decay_prob = 1 - np.exp(-1 / sister_tau)
    ...
    # Clear trajectories
    # translocator1.clear_trajectory()
    # translocator2.clear_trajectory()
...
def run_synchronized_trajectories_continue(translocator1, translocator2, period=None, steps=None, 
                                         prune_unbound_LEFs=True, track_sisters=False, 
...
def disable_sister_updates_in_step(translocator):
    ...
    translocator.extrusion_engine.update_sister_states = lambda: None
    return original_update

disable_sister_updates_in_step(translocator1)
disable_sister_updates_in_step(translocator2)

apply_param_dict_to_translocator(translocator1, paramdict_WT)
apply_param_dict_to_translocator(translocator2, paramdict_WT)
...
print(f"After first run: {len(translocator1.state_trajectory)} trajectory points")
apply_param_dict_to_translocator(translocator1, paramdict_dN)
apply_param_dict_to_translocator(translocator2, paramdict_dN)

# Second run - continue from where we left off
run_synchronized_trajectories_continue(
    translocator1, translocator2,
    steps = 57600,  # Different parameters
    prune_unbound_LEFs = False,
    track_sisters = True,
    sample_interval = 50,
    seed = 123,
    clear_trajectories = False  # Keep existing data
)

print(f"After second run: {len(translocator1.state_trajectory)} trajectory points")
...
import pickle
with open('translocator1_trajectory.pkl', 'wb') as f:
    pickle.dump({
...
with open('translocator2_trajectory.pkl', 'wb') as f:
    pickle.dump({
