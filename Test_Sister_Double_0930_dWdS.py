...
with open("data/new_dict/alpha50_tau10h/extrusion_dict_RN_RB_RP_RW_HBD_dS4h_alpha50_tau10h.json", 'r') as dict_file:
        paramdict_dS9h = json.load(dict_file)

with open("data/new_dict/alpha50_tau10h/extrusion_dict_RN_RB_RP_RW_HBD_dW13h_alpha50_tau10h.json", 'r') as dict_file:
...
LEF_off_rate = paramdict_dS9h['LEF_off_rate']
CTCF_facestall = paramdict_dS9h['CTCF_facestall']
...
run_synchronized_trajectories_continue(
    ...
    paramdict_dS9h['sister_lifetime'],
    steps = 28800, 
...
update(translocator1, paramdict_dWdS)
update(translocator2, paramdict_dWdS)

# Second run - continue from where we left off
run_synchronized_trajectories_continue(
   translocator1, translocator2, 
   paramdict_dWdS['sister_lifetime'],
...
with open('dWdS_trajectory1.pkl', 'wb') as f:
    pickle.dump({
...
with open('dWdS_trajectory2.pkl', 'wb') as f:
    pickle.dump({
