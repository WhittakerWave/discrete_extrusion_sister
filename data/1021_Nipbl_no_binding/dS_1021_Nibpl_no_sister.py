...
def run_simulation(config, residence_time, sister_damping):
    ...
    """
    NUM_SISTERCS = int(7765 * np.exp(7/residence_time))
    ...
    LEF_sep_12h = int(LATTICE_SIZE * extC_bound_frac / (extC_value / 2))
    
    total_sister_rad21 = (df_dS['RacPS'][index] + df_dS['RacPW'][index] + 
                          df_dS['RacP'][index] + df_dS['Rac'][index])

    sister_RAD21_time_12h = calculate_sister_RAD21_bound_time(
    ...
    output_params["LEF_separation"] = LEF_sep_12h
    ...
    output_params["sister_lifetime"] = int(sister_RAD21_time_12h)
...
def main():
    ...
    # Load configuration
    config = load_config('network_parameters_dS99.json')
