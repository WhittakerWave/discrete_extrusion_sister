...
### 
### This script generates the parameters for Nipbl depletion
import json
import numpy as np
import sympy as sym 
...
def run_simulation(config, residence_time, sister_damping, collision_prob):
    """
    Run the simulation for given residence_time, sister_damping and collision release prob
    ...
    ## Apply Nipbl depletion at 2h since the boundary of S/G2
    time_2h = 3600 * 2 - 1
    ...
    output_params["collision_release_prob"] = collision_prob
...
def main():
    ...
    for residence_time in RESIDENCE_TIMES:
        for sister_damping in SISTER_DAMPINGS:
            for k, collision_prob in enumerate(COLLISION_PROB):
                print(f"\nRunning: residence_time={residence_time}h, sister_damping={sister_damping}, collision_prob={collision_prob}")
                
                # Run simulation
                params = run_simulation(config, residence_time, sister_damping, collision_prob)
            
                # Save to file
                filename = (f"{config['output_prefix']}_"
                        f"alpha{sister_damping}_tau{residence_time}h_beta{k}.json")
