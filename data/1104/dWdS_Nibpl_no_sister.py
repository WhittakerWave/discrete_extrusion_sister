...
def run_simulation(config, residence_time, bypass_probs):
    ...
    Model_ext_coh_dS = r_ext_coh.simulate(0, 3600*14, 3600*14)
    ...
    init_conditions_ext_coh_dWapl_14h = {
        Rac_free_init: df_dS['Rac_free'][time_14h], 
        Rac_init: df_dS['Rac'][time_14h], 
        RacP_init: df_dS['RacP'][time_14h] + df_dS['RacPW'][time_14h] * depletion_level_wapl,
        RacPW_init: df_dS['RacPW'][time_14h] * remaining_level_wapl,
        RacPS_init: df_dS['RacPS'][time_14h],

        R_free_init: df_dS['R_free'][time_14h],
        RN_init: df_dS['RN'][time_14h] ,
        R_init: df_dS['R'][time_14h]  ,
        RP_init: df_dS['RP'][time_14h] + df_dS['RPW'][time_14h] * depletion_level_wapl, 
        RPW_init: df_dS['RPW'][time_14h] * remaining_level_wapl, 
    
        N_init: df_dS['N'][time_14h],
        S_init: df_dS['S'][time_14h],
        W_init: df_dS['W'][time_14h] * remaining_level_wapl,
        P_init: df_dS['P'][time_14h],
       }

    paras_dict_since_14h_dW = paras_dict_coh_local | paras_dict_ext_local | init_conditions_ext_coh_dWapl_14h | {"K_Rac_free_R_free": 1/2495}
    paras_dict_ext_coh_since_14h_dW = {str(key): value for key, value in paras_dict_since_14h_dW.items()}
    
    Model_ext_coh_since_14h_dW = build_model(MODEL_EXT_COH_TEMPLATE, paras_dict_ext_coh_since_14h_dW)
    # print(model_ext_coh)
    # Load the modes
    r_ext_coh_since_14h_dW = te.loada(Model_ext_coh_since_14h_dW)
    # Simulate the model
    Model_ext_coh_since_14h_dW = r_ext_coh_since_14h_dW.simulate(0, 3600*10, 3600*10)
    
    df_since_14h = pd.DataFrame(Model_ext_coh_since_14h_dW, columns=columns)
    
    analysis_hours = config['simulation_parameters']['analysis_timepoint_hours'] 
    index = 3600 * analysis_hours  - 1

    # Calculate metrics
    total_bound_ext = (df_since_14h['R'][index] + df_since_14h['RN'][index] + 
                       df_since_14h['RP'][index] + df_since_14h['RPW'][index])
    ...
    rates = {
        'R_free_to_RN': paras_dict_ext_coh_since_14h_dW['Kext_R_free_RN']*df_since_14h['N'][index],
        'RPW_R_free': paras_dict_ext_coh_since_14h_dW['Kext_RPW_R_free'],
        'RN_R': paras_dict_ext_coh_since_14h_dW['Kext_RN_R'],
        'R_RN': paras_dict_ext_coh_since_14h_dW['Kext_R_RN']*df_since_14h['N'][index],
        'R_RP': paras_dict_ext_coh_since_14h_dW['Kext_R_RP']*df_since_14h['P'][index],
        'RP_R': paras_dict_ext_coh_since_14h_dW['Kext_RP_R'],
        'RP_RPW': paras_dict_ext_coh_since_14h_dW['Kext_RP_RPW']*df_since_14h['W'][index],
        'RPW_RP': paras_dict_ext_coh_since_14h_dW['Kext_RPW_RP'],
    }
    ...
    output_params["bypass_probs"] = bypass_probs
