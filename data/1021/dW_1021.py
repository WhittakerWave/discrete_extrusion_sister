...
def run_simulation(config, residence_time, sister_damping):
    ...
    init_conditions_ext_coh_dWapl_7h = {
        Rac_free_init: df_WT['Rac_free'][time_7h], 
        Rac_init: df_WT['Rac'][time_7h], 
        RacN_init: df_WT['RacN'][time_7h], 
        RacP_init: df_WT['RacP'][time_7h] + df_WT['RacPW'][time_7h] * depletion_level,
        RacPW_init: df_WT['RacPW'][time_7h] * remaining_level,
        RacPS_init: df_WT['RacPS'][time_7h],

        R_free_init: df_WT['R_free'][time_7h],
        RN_init: df_WT['RN'][time_7h] ,
        R_init: df_WT['R'][time_7h]  ,
        RP_init: df_WT['RP'][time_7h] + df_WT['RPW'][time_7h] * depletion_level, 
        RPW_init: df_WT['RPW'][time_7h] * remaining_level, 
    
        N_init: df_WT['N'][time_7h],
        S_init: df_WT['S'][time_7h],
        W_init: df_WT['W'][time_7h] * remaining_level,
        P_init: df_WT['P'][time_7h],
       }

    paras_dict_since_7h_dW = paras_dict_coh_local | paras_dict_ext_local | init_conditions_ext_coh_dWapl_7h | {"K_Rac_free_R_free": 1/2495}
    paras_dict_ext_coh_since_7h_dW = {str(key): value for key, value in paras_dict_since_7h_dW.items()}
    
    Model_ext_coh_since_7h_dW = build_model(MODEL_EXT_COH_TEMPLATE, paras_dict_ext_coh_since_7h_dW)
    # print(model_ext_coh)
    # Load the modes
    r_ext_coh_since_7h_dW = te.loada(Model_ext_coh_since_7h_dW)
    # Simulate the model
    Model_ext_coh_since_7h_dW = r_ext_coh_since_7h_dW.simulate(0, 3600*10, 3600*10)
    
    df_since_7h = pd.DataFrame(Model_ext_coh_since_7h_dW, columns=columns)
    
    analysis_hours = config['simulation_parameters']['analysis_timepoint_hours'] 
    index = 3600 * analysis_hours  - 1

    # Calculate metrics
    total_bound_ext = (df_since_7h['R'][index] + df_since_7h['RN'][index] + 
                       df_since_7h['RP'][index] + df_since_7h['RPW'][index])
    
    bound_extC_ratio = total_bound_ext / (paras_dict_coh_local[N_R]*0.5)
    extC_bound_frac = total_bound_ext / (total_bound_ext + df_since_7h['R_free'][index])
    ...
    total_sister_rad21 = (df_since_7h['RacPS'][index] + df_since_7h['RacPW'][index] + 
                          df_since_7h['RacP'][index] + df_since_7h['Rac'][index] + 
                          df_since_7h['RacN'][index])

    sister_RAD21_time_10h = calculate_sister_RAD21_bound_time(
        K_RacPW_Rac_free = paras_dict_ext_coh_since_7h_dW['K_RacPW_Rac_free'], \
        B_W_sister = df_since_7h['RacPW'][index], \
        B_R_sister = total_sister_rad21)
    
    total_extrusive_rad21 = (df_since_7h['RPW'][index] + 
                          df_since_7h['RP'][index] + df_since_7h['R'][index] + 
                          df_since_7h['RN'][index])

    extruder_RAD21_time_10h = calculate_extrusive_RAD21_bound_time(
        K_RPW_R_free = paras_dict_ext_coh_since_7h_dW['Kext_RPW_R_free'], \
        B_W_extruder = df_since_7h['RPW'][index], \
        B_R_extruder = total_extrusive_rad21)
    
    # Calculate transition rates
    rates = {
        'R_free_to_RN': paras_dict_ext_coh_since_7h_dW['Kext_R_free_RN']*df_since_7h['N'][index],
        'RPW_R_free': paras_dict_ext_coh_since_7h_dW['Kext_RPW_R_free'],
        'RN_R': paras_dict_ext_coh_since_7h_dW['Kext_RN_R'],
        'R_RN': paras_dict_ext_coh_since_7h_dW['Kext_R_RN']*df_since_7h['N'][index],
        'R_RP': paras_dict_ext_coh_since_7h_dW['Kext_R_RP']*df_since_7h['P'][index],
        'RP_R': paras_dict_ext_coh_since_7h_dW['Kext_RP_R'],
        'RP_RPW': paras_dict_ext_coh_since_7h_dW['Kext_RP_RPW']*df_since_7h['W'][index],
        'RPW_RP': paras_dict_ext_coh_since_7h_dW['Kext_RPW_RP'],
    }
