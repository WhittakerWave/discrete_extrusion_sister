...
def run_simulation(config, residence_time, sister_damping):
    ...
    Model_ext_coh_WT = r_ext_coh.simulate(0, 3600*17, 3600*17)
    ...
    init_conditions_ext_coh_dNipbl_1h = {
        Rac_free_init: df_WT['Rac_free'][time_1h], 
        Rac_init: df_WT['Rac'][time_1h] + df_WT['RacN'][time_1h] * depletion_level, 
        RacN_init: df_WT['RacN'][time_1h] * remaining_level, 
        RacP_init: df_WT['RacP'][time_1h],
        RacPW_init: df_WT['RacPW'][time_1h],
        RacPS_init: df_WT['RacPS'][time_1h],

        R_free_init: df_WT['R_free'][time_1h],
        RN_init: df_WT['RN'][time_1h] * remaining_level ,
        R_init: df_WT['R'][time_1h] + df_WT['RN'][time_1h] * depletion_level ,
        RP_init: df_WT['RP'][time_1h], 
        RPW_init: df_WT['RPW'][time_1h], 
    
        N_init: df_WT['N'][time_1h] * remaining_level,
        S_init: df_WT['S'][time_1h],
        W_init: df_WT['W'][time_1h],
        P_init: df_WT['P'][time_1h],
       }

    paras_dict_since_1h_dN = paras_dict_coh_local | paras_dict_ext_local | init_conditions_ext_coh_dNipbl_1h | {"K_Rac_free_R_free": 1/2495}
    paras_dict_ext_coh_since_1h_dN = {str(key): value for key, value in paras_dict_since_1h_dN.items()}
    
    Model_ext_coh_since_1h_dN = build_model(MODEL_EXT_COH_TEMPLATE, paras_dict_ext_coh_since_1h_dN)
    # print(model_ext_coh)
    # Load the modes
    r_ext_coh_since_1h_dN = te.loada(Model_ext_coh_since_1h_dN)
    # Simulate the model
    Model_ext_coh_since_1h_dN = r_ext_coh_since_1h_dN.simulate(0, 3600*16, 3600*16)
    
    df_since_1h = pd.DataFrame(Model_ext_coh_since_1h_dN, columns=columns)
    
    analysis_hours = config['simulation_parameters']['analysis_timepoint_hours'] 
    index = 3600 * analysis_hours  - 1 

    # Calculate metrics
    total_bound_ext = (df_since_1h['R'][index] + df_since_1h['RN'][index] + 
                       df_since_1h['RP'][index] + df_since_1h['RPW'][index])
    
    bound_extC_ratio = total_bound_ext / (paras_dict_coh_local[N_R]*0.5)
    extC_bound_frac = total_bound_ext / (total_bound_ext + df_since_1h['R_free'][index])
    ...
    total_sister_rad21 = (df_since_1h['RacPS'][index] + df_since_1h['RacPW'][index] + 
                          df_since_1h['RacP'][index] + df_since_1h['Rac'][index] + 
                          df_since_1h['RacN'][index])

    sister_RAD21_time_10h = calculate_sister_RAD21_bound_time(
        K_RacPW_Rac_free = paras_dict_ext_coh_since_1h_dN['K_RacPW_Rac_free'], \
        B_W_sister = df_since_1h['RacPW'][index], \
        B_R_sister = total_sister_rad21)
    
    # Calculate transition rates
    rates = {
        'R_free_to_RN': paras_dict_ext_coh_since_1h_dN['Kext_R_free_RN']*df_since_1h['N'][index],
        'RPW_R_free': paras_dict_ext_coh_since_1h_dN['Kext_RPW_R_free'],
        'RN_R': paras_dict_ext_coh_since_1h_dN['Kext_RN_R'],
        'R_RN': paras_dict_ext_coh_since_1h_dN['Kext_R_RN']*df_since_1h['N'][index],
        'R_RP': paras_dict_ext_coh_since_1h_dN['Kext_R_RP']*df_since_1h['P'][index],
        'RP_R': paras_dict_ext_coh_since_1h_dN['Kext_RP_R'],
        'RP_RPW': paras_dict_ext_coh_since_1h_dN['Kext_RP_RPW']*df_since_1h['W'][index],
        'RPW_RP': paras_dict_ext_coh_since_1h_dN['Kext_RPW_RP'],
    }
