...
with open("extrusion_dict_Simple_6.json", 'r') as dict_file:
...
translocator1 = Translocator_Sister(BaseExtruder_Sister,
                    ...
                    initial_sister_positions = np.array([2500, 3500, 4500, 4500, 4500, 4500]),
                    **paramdict)

# translocator1.run(10000)
translocator1.run_trajectory_one_sister(steps = 10000, prune_unbound_LEFs=True, track_sisters=True, sample_interval=1)

end = time.time()
print(f"Run time: {end - start:.2f} seconds")
print(f"Before manual init: num_sisters = {translocator1.extrusion_engine}")

with open('test_case_6.pkl', 'wb') as f:
    pickle.dump({
