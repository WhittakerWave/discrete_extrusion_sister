...
# ---- Per-extruder expected N_ex(t) with collisions ----
def expected_N_ex(t, rho, alpha, rho_c, rho_e, v=1.0):
    """
    Expected number of collected points per extruder at time t.
    rho    = M/L
    alpha  = extrusion difficulty
    rho_c  = expected points at seed site
    rho_e  = extruder density = k/L
    v      = extrusion speed
    """
    lam = 2.0 * rho_e * v
    if lam == 0.0:
    ...
    # first term: still active at time t
    term1 = np.exp(-lam * t) * Ns(t, rho, alpha, rho_c)

    # second term: truncated by collision at tau
...
# ---- Total expected N(t) ----
def N_total_collision_integral(t_array, M, L, k, rho, alpha, rho_c=None, v=1.0):
    if rho_c is None:
        rho_c = rho
    rho_e = k / L
    out = np.zeros_like(t_array, dtype=float)
    for i, t in enumerate(t_array):
        N_ex = expected_N_ex(t, rho, alpha, rho_c, rho_e, v=v)
        out[i] = min(k * N_ex, M)
    return out
...
filenames = [
    "test_simple_formula_0926/test_case_simple_alpha=10_k=500_run1.pkl",
    "test_simple_formula_0926/test_case_simple_alpha=10_k=500_run2.pkl",
    "test_simple_formula_0926/test_case_simple_alpha=10_k=500_run3.pkl",
]
